-- This set of exercises will focus on an auction. Create a new database 
-- called auction. In this database there will be three tables, bidders, 
-- items, and bids.
...
-- id of type SERIAL: this should be a primary key
-- name of type text: this should be NOT NULL
-- initial_price and sales_price: These two columns should both be of 
-- type numeric. Each column should be able to hold a number as high as 
-- 1000 dollars with 2 decimal points of precision.
-- The initial_price represents the starting price of an item when it is 
-- first put up for auction. This column should never be NULL.
-- The sales_price represents the final price at which the item was sold. 
-- This column may be NULL, as it is possible to have an item that was never 
-- sold off.
-- bids
-- id of type SERIAL: this should be a primary key
