...
SELECT name, (SELECT COUNT(item_id) FROM bids WHERE bids.item_id = items.id)
FROM items;

-- If we wanted to get an equivalent result, without using a subquery, then we would have 
-- to use a LEFT OUTER JOIN. Can you come up with the equivalent query that uses a JOIN clause?
